        ...
        customFetch("john", (d) => {
            if (d["john"]["creator"] == that.props.user.identifier || d["players"].includes(that.props.user.identifier) || d["john"]["pendingPlayers"].includes(that.props.user.identifier)) {
                that.setState({
                    ...
                    ownJohn: d["john"]["creator"] == that.props.user.identifier,
                    fullPlayers: d["fullPlayers"],
                    players: d["players"],
                    playersNames: d["playersNames"],
                    pendingPlayersNames: d["pendingPlayersNames"],
                    inJohn: true
                })
            ...
            {(this.state.inJohn ?
                (<div>
                    ...
                    {(this.state.john.creator.toString() == this.props.user.identifier || this.state.players.includes(this.props.user.identifier as never) ?
                            (<div>
                                <Players matchmaker={this} user={this.props.user}
                                         matchmakingState={this.state}></Players>
                                <hr/>
                                <PlayerView ref={this.playerViewRef} ownJohn={this.state.ownJohn}
                                            user={this.props.user}></PlayerView>
                            </div>)
                            : (<div></div>)
                    )}
                </div>) :
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <input onChange={(event) => {
                                    this.setState({joinIdentifier: event.target.value})
                                }} placeholder="000000"/>
                                <br/>
                                <button type="button" onClick={() => {
                                    this.joinJohn()
